...
def read_datat1w_score_hcpa(data_dir):
    onlyfiles = [f for f in listdir(data_dir) if osp.isfile(osp.join(data_dir, f))]
    onlyfiles.sort()
    batch = []
    pseudo = []
    ...
    pool = multiprocessing.Pool(processes=cores)
    #pool =  MyPool(processes = cores)
    func = partial(read_sigle_hcpt1w_data_score_hcpa, data_dir)
...
def read_datafmri_score(data_dir):
    ...
    # func = partial(read_sigle_hcpfmri_data_score_sbjnum, data_dir)
...
def read_datat1w_sbjnum_score(data_dir):
    onlyfiles = [f for f in listdir(data_dir) if osp.isfile(osp.join(data_dir, f))]
    onlyfiles.sort()
    batch = []
    pseudo = []
    ...
    pool = multiprocessing.Pool(processes=cores)
    #pool =  MyPool(processes = cores)
    func = partial(read_sigle_hcpt1w_data_score_sbjnum, data_dir)
...
#####################################
    ...
    data = Data(x=att_torch, edge_index=edge_index_torch, y=y_torch, edge_attr=edge_att_torch, pos=pseudo_torch,
                sbj_fname=sbj_fname, edge_sbj_torch=edge_sbj_torch)
...
def read_datat1w_sbjnum_score_hcpa(data_dir):
    onlyfiles = [f for f in listdir(data_dir) if osp.isfile(osp.join(data_dir, f))]
    onlyfiles.sort()
    batch = []
    pseudo = []
    ...
    pool = multiprocessing.Pool(processes=cores)
    #pool =  MyPool(processes = cores)
    func = partial(read_sigle_hcpt1w_data_score_sbjnum_hcpa, data_dir)
...
def read_datafmri_sbjnum_score_hcpa(data_dir):
    onlyfiles = [f for f in listdir(data_dir) if osp.isfile(osp.join(data_dir, f))]
    onlyfiles.sort()
    batch = []
    pseudo = []
    ...
    pool = multiprocessing.Pool(processes=cores)
    #pool =  MyPool(processes = cores)
    func = partial(read_sigle_hcpafmri_data_score_sbjnum, data_dir)
...
def read_sigle_hcpt1w_data_score_hcpa(data_dir,filename,use_gdc =False):
...
def read_sigle_hcpt1w_data_score_sbjnum_hcpa(data_dir,filename,use_gdc =False):
    import scipy.io

    temp = dd.io.load(osp.join(data_dir, filename))
    feature = np.genfromtxt(osp.join(data_dir[:-4], filename[:-3], filename[:-3]+'_ROI_feature_Gordon333.txt'))

    # adj_matrix = np.genfromtxt(osp.join(data_dir[:-4], filename[:-3], filename[:-3]+'_ROI_adj_Gordon333.txt'))
    adj_matrix = scipy.io.loadmat(osp.join(data_dir[:-4], filename[:-3], filename[:-3] + '_Gordon333_correlation.mat'))
    adj_matrix = adj_matrix['connectivity']
    label_all = np.genfromtxt(osp.join(data_dir[:-18], 'interview_age.txt'))
    # read edge and edge attribute
    ...
    att = feature #temp['corr'][()]
    # label = temp['label'][()]
    label = label_all[np.where(label_all==int(filename[3:-3]))[0],1]
...
def read_sigle_hcpfmri_data_score_sbjnum(data_dir,filename,use_gdc =False):
    temp = dd.io.load(osp.join(data_dir, filename))
...
def read_sigle_hcpafmri_data_score_sbjnum(data_dir,filename,use_gdc =False):
    temp = dd.io.load(osp.join(data_dir, filename))
    # temp['corr'] = temp['corr'][1:, 1:]
    # temp['pcorr'] = temp['pcorr'][1:, 1:]
    ...
    label_all = np.genfromtxt(osp.join(data_dir[:-25], 'interview_age.txt'))
    # ProcSpeed_AgeAdj.txt     ReadEng_Unadj.txt PMAT24_A_CR.txt
    # read edge and edge attribute
    pcorr = temp['pcorr'][()]
    pcorr[pcorr < 0] = 0
    ...
    label = label_all[np.where(label_all==int(filename[3:-8]))[0],1]
