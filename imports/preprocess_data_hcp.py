...
# root_folder = '/data/hzb/project/Brain_Predict_Score/BrainGNN/BrainGNN_Pytorch-main/data_hcp/'
# root_folder = '/data/hzb/project/Brain_Predict_Score/data_hcp_4fmri/'




# root_folder = '/data/hzb/project/BodyDecoding/data/ABCD_pcp/'
# data_folder = os.path.join(root_folder, 'Schaefer/filt_noglobal/')
# phenotype = os.path.join(root_folder, 'ABCD_Phenotype/ABCD_Phenotype_end_1110.csv')


# root_folder = '/data/hzb/project/BodyDecoding/data/HCP_A_pcp/'
# data_folder = os.path.join(root_folder, 'Schaefer/filt_noglobal/')
# phenotype = os.path.join(root_folder, 'HCP_A_Phenotype/1223_HCP_A_Phenotype_Value_34.csv')

root_folder = '/data/hzb/project/BodyDecoding/data/HCP_D_pcp/'
data_folder = os.path.join(root_folder, 'Schaefer/filt_noglobal/')
phenotype = os.path.join(root_folder, 'HCP_D_Phenotype/HCP_D_first_select_72.csv')
...
def get_ids_abcd(num_subjects=None):
    """
    return:
        subject_IDs    : list of all subject IDs
    """
    data_folder = '/data/hzb/project/BodyDecoding/data/ABCD_pcp/Schaefer/filt_noglobal/'
    subject_IDs = np.genfromtxt(os.path.join(data_folder, 'subject_IDs.txt'), dtype=str)
...
def get_ids_hcp_a(num_subjects=None):
    """
    return:
        subject_IDs    : list of all subject IDs
    """
    data_folder = '/data/hzb/project/BodyDecoding/data/HCP_A_pcp/Schaefer/filt_noglobal/'
    subject_IDs = np.genfromtxt(os.path.join(data_folder, 'subject_IDs.txt'), dtype=str)
...
def get_ids_hcp_d(num_subjects=None):
    """
    return:
        subject_IDs    : list of all subject IDs
    """
    ...
    with open(phenotype) as csv_file:
        ...
        for row in reader:
            if row['Subject'] in subject_list:
                scores_dict[row['Subject']] = 1

                # if row[score] == 'M':
                #     scores_dict[row['Subject']]=0
                # else:
                #     scores_dict[row['Subject']]=1
...
def get_subject_score_abcd(subject_list, score):
    scores_dict = {}

    with open(phenotype) as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            if row['NAME'] in subject_list:
                scores_dict[row['NAME']] = 1
...
def get_subject_score_hcp_a(subject_list, score):
    scores_dict = {}

    with open(phenotype) as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            if row['subject_id_fmri'] in subject_list:
                scores_dict[row['subject_id_fmri']] = 1
...
def get_subject_score_hcp_d(subject_list, score):
    scores_dict = {}

    with open(phenotype) as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            if row['sbj_name'] in subject_list:
                scores_dict[row['sbj_name']] = 1
...
def get_subject_score_4fmri(subject_list, score):
    scores_dict = {}
    subject_list2 = subject_list

    # for i in range(subject_list2.size):
    #     subject_list2[i] = subject_list2[i][:-5]

    with open(phenotype) as csv_file:
        reader = csv.DictReader(csv_file)
        for row in reader:
            if row['rsfmrilist'] in subject_list2:
                if row[score] == 'M':
                    scores_dict[row['rsfmrilist']]=0
                else:
                    scores_dict[row['rsfmrilist']]=1
