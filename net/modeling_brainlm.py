...
class BrainLMDecoder_mask(ViTMAEDecoder):
    ...
    def random_masking(self, sequence, istraining, rand_index):    #zhibin
        """
        Perform per-sample random masking by per-sample shuffling. Per-sample shuffling is done by argsort random
        noise.
        """
        ...
        if istraining:
            ...
            # (省略硬编码修复后的 even_numbers 和 mask 定义)
            even_numbers = torch.tensor([
                torch.randint(0, seq_length // 2, (1,)) * 2
            ...
            # (省略硬编码修复后的 even_numbers 和 mask 定义)
            even_numbers = torch.full((batch_size,), rand_index)
            mask = torch.zeros([batch_size, seq_length], device=sequence.device)
            for i in range(mask.shape[0]):
                mask[i, even_numbers[i]] = 1
