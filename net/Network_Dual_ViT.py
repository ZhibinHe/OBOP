...
class Network_regress_score_512(torch.nn.Module):
    def __init__(self, fmri_indim, fmri_outdim, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim):
        '''
        ...
        :param R: (int) number of ROIs
        '''
        super(Network_regress_score_512, self).__init__()
        self.fmri_indim = fmri_indim
        self.fmri_outdim = fmri_outdim
        self.image_size = image_size
        self.patch_size = patch_size
...
# class Network_regress_score_out333(torch.nn.Module):
#     def __init__(self, fmri_indim, fmri_outdim, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim):
#         '''
#
#         :param indim: (int) node feature dimension
...
#         :param R: (int) number of ROIs
#         '''
#         super(Network_regress_score_out333, self).__init__()
#         self.fmri_indim = fmri_indim
#         self.fmri_outdim = fmri_outdim
#         self.image_size = image_size
#         self.patch_size = patch_size
...
#         self.fc1 = torch.nn.Linear(self.fmri_indim, 512)
#         self.fc2 = torch.nn.Linear(512, 1024)
#         self.model_v = ViT_nomean(image_size = self.image_size, patch_size = self.patch_size, num_classes = self.num_classes, dim = self.dim, depth = self.depth, heads = self.heads, mlp_dim = self.mlp_dim)
#         self.fc3 = torch.nn.Linear(1024, 512)
...
#         self.bn2 = torch.nn.BatchNorm1d(1024)
#
#         # self.cov1 = ConvReLU(1, 4, 3, padding=1)
#         # self.proj = nn.Linear(8*333, 512)
#
#     def forward(self, x):
#         # x = self.bn1(F.relu(self.fc1(x)))
#         # x = self.cov1(x.unsqueeze(1)) # 8, 8, 333, 333
#
#         # x = x*2-1
#         x = F.relu(self.fc1(x))
#         x = F.dropout(x, p=0.6, training=self.training)
...
#         # bs, c, h, w = x.size()
#         # x = x.permute(0, 2, 1, 3).contiguous().view(bs, h, c*w)
#         # x = self.proj(x)
#         # import ipdb; ipdb.set_trace()
#         ######################################
#         # x = F.relu(self.fc1(x)) # 333 x 333
#         # x = F.dropout(x, p=0.6, training=self.training)
#         # x = self.bn1(x.transpose(1, 2)).transpose(1, 2)
#         # # x = F.relu(self.fc2(x))
#         # # x = F.dropout(x, p=0.6, training=self.training)
...
class Network_regress_score_out400(torch.nn.Module):
    def __init__(self, fmri_indim, fmri_outdim, image_size, patch_size, num_classes, dim, depth, heads, mlp_dim):
        '''
        ...
        :param R: (int) number of ROIs
        '''
        super(Network_regress_score_out400, self).__init__()
        self.fmri_indim = fmri_indim
        self.fmri_outdim = fmri_outdim
        self.image_size = image_size
        self.patch_size = patch_size
