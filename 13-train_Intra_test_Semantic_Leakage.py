...
USAGE:
    1. Training & Testing (Default):
       ...
       python ./13-train_Intra_test_Semantic_Leakage.py --dataset HCP_A_intra_SL  --num_prediction 21 --zero_num_prediction 13 --gpu_id 0
       python ./13-train_Intra_test_Semantic_Leakage.py --dataset HCP_D_intra_SL  --num_prediction 20 --zero_num_prediction 14 --gpu_id 0
       python ./13-train_Intra_test_Semantic_Leakage.py --dataset HCP_YA_intra_SL  --num_prediction 24 --zero_num_prediction 11 --gpu_id 0


    2. Test Only (Skip Training, load existing models):
       python ./13-train_Intra_test_Semantic_Leakage.py --test_only --dataset ABCD_intra_SL  --num_prediction 76 --zero_num_prediction 39 --gpu_id 0
       python ./13-train_Intra_test_Semantic_Leakage.py --test_only --dataset HCP_A_intra_SL  --num_prediction 21 --zero_num_prediction 13 --gpu_id 0
       python ./13-train_Intra_test_Semantic_Leakage.py --test_only --dataset HCP_D_intra_SL  --num_prediction 20 --zero_num_prediction 14 --gpu_id 0
       python ./13-train_Intra_test_Semantic_Leakage.py --test_only --dataset HCP_YA_intra_SL  --num_prediction 24 --zero_num_prediction 11 --gpu_id 0


INPUTS:
    - Dataset Name: Selection of 'ABCD_intra_SL', 'HCP_D_intra_SL', 'HCP_A_intra_SL', or 'HCP_YA_intra_SL'.
...
OUTPUTS:
    - Model Checkpoints:
        - SL_Intra_best_stage1_model_train_<dataset>.pth (Supervised encoder weights)
        - SL_Intra_best_stage2_model_test_<dataset>.pth (Generative decoder weights)
    - Test Results (.npy):
        - results_SL_Intra_<dataset>_sys_score.npy (Zero-shot predicted phenotype scores)
        - results_SL_Intra_<dataset>_label_score.npy (Ground truth phenotype scores)
        - results_SL_Intra_<dataset>_pcc_per_task.npy (Pearson Correlation for each unseen task)
        - results_SL_Intra_<dataset>_cod_per_task.npy (Coefficient of Determination/R2 for each unseen task)
=========================================================
"""
...
parser.add_argument('--dataset', type=str, default='HCP_A_intra_SL', choices=['ABCD_intra_SL', 'HCP_D_intra_SL', 'HCP_A_intra_SL', 'HCP_YA_intra_SL'],
...
parser.add_argument('--num_prediction', type=int, default=21, help='Dimension of phenotype scores (labels)') #76,39; 21, 13; 20, 14; 24, 11
parser.add_argument('--zero_num_prediction', type=int, default=13, help='number prediction')
...
def load_data(dataset_name, opt):
    ...
    if  dataset_name == 'ABCD_intra_SL':
        ...
        return data_load_abcd_intra_SL(dataroot, dataset_name, opt)


    elif dataset_name == 'HCP_A_intra_SL':
        ...
        return data_load_hcp_a_intra_SL(dataroot, dataset_name, opt)


    elif dataset_name == 'HCP_D_intra_SL':
        ...
        return data_load_hcp_d_intra_SL(dataroot, dataset_name, opt)

    elif dataset_name == 'HCP_YA_intra_SL':
        ...
        return data_load_hcp_ya_intra_SL(dataroot, dataset_name, opt)
...
def run_stage1(train_loader, val_loader, text_feature):
    ...
    best_pcc = -1.0
    best_model_path = os.path.join(opt.save_path, f'SL_Intra_best_stage1_model_train_{opt.dataset}.pth')
...
def run_stage2(train_feat, train_rw, train_labels, text_feature):      ##biaozhun
    ...
    best_loss = float('inf')
    best_model_path = os.path.join(opt.save_path, f'SL_Intra_best_stage2_model_test_{opt.dataset}.pth')
...
if __name__ == '__main__':
    ...
    if not opt.test_only:
        ...
        # Define model paths
        best_stage1_path = os.path.join(opt.save_path, f'SL_Intra_best_stage1_model_train_{opt.dataset}.pth')
        best_stage2_path = os.path.join(opt.save_path, f'SL_Intra_best_stage2_model_test_{opt.dataset}.pth')
    ...
    if  opt.test_only:
        ...
        # 6. Save results
        base_name = f'results_SL_Intra_{opt.dataset}'
