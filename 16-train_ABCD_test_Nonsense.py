...
parser.add_argument('--target_dataset', type=str, default='HCP_YA', choices=['HCP_D', 'HCP_A', 'HCP_YA'],
...
def extract_tgt_features(stage1_model_path, loader, text_feature_tgt):
    ...
    with torch.no_grad():
      ...
      for data in loader:
        ...
        text_feature_tmp[0:10, :] = text_feature_tgt
        score_predict, regress_weight, feature_cat = model1(data.x.view(int(data.x.shape[0] / opt.nroi), opt.nroi, opt.nroi), text_feature_tmp)
...
if __name__ == '__main__':
    ...
    if not  opt.test_only:
        ...
        text_feature_N = torch.load(os.path.join(opt.phenoroot + '/Nonsense_Phenotype_Description.pt'), map_location=device)
        text_feature_N = text_feature[0:text_feature_N.shape[0], :]


        tgt_fusion_feat, tgt_labels = extract_tgt_features(best_stage1_path, test_loader_tgt, text_feature_N)
