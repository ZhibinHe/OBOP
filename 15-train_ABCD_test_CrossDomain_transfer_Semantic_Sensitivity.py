...
parser.add_argument('--target_dataset', type=str, default='HCP_A', choices=['HCP_D', 'HCP_A', 'HCP_YA'],
...
parser.add_argument('--semantic_sensitivity', type=int, default=1, choices=[1, 2, 3])
...
parser.add_argument('--zero_num_prediction', type=int, default=34, help='number prediction') #34, 34, 35
...
def load_data(dataset_name, opt):
    ...
    if dataset_name == 'HCP_A':
        ...
        return data_load_hcp_a_ss(dataroot, dataset_name, opt)
        ...
        return data_load_hcp_d_ss(dataroot, dataset_name, opt)
        ...
        return data_load_hcp_ya_ss(dataroot, dataset_name, opt)
...
def run_stage2(train_feat, train_rw, train_labels, text_feature):      ##biaozhun
    ...
    best_loss = float('inf')
    best_model_path = os.path.join(opt.save_path, f'best_stage2_model_test_{opt.target_dataset}_SS_{opt.semantic_sensitivity}.pth')
...
if __name__ == '__main__':
    ...
    if not opt.test_only:
        ...
        best_stage2_path = os.path.join(opt.save_path, f'best_stage2_model_test_{opt.target_dataset}_SS_{opt.semantic_sensitivity}.pth')
    ...
    if  opt.test_only:
        ...
        # 6. Save results
        base_name = f'SS_{opt.semantic_sensitivity}_results_{opt.source_dataset}_to_{opt.target_dataset}'
